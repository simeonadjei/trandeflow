...
function SuperBotPanel({ session, goldenWindow }: { session: any; goldenWindow: any }) {
  ...
  const gwWeekday: string | null = session?.goldenWeekdayLabel ?? goldenWindow.goldenWeekdayLabel ?? null;
  const aboveFloor: boolean = session?.aboveFloor ?? goldenWindow.aboveFloor ?? true;
  const bothConfirmed: boolean = session?.bothAssetsConfirmed ?? false;

  const inWindow = session?.inGoldenWindow ?? false;
  const minsTo = session?.minsToGoldenWindow ?? 0;
  const hoursTo = Math.floor(minsTo / 60);
  const daysTo  = Math.floor(hoursTo / 24);
  ...
  const slotLabel = gwWeekday
    ? `${gwWeekday}s ${String(gh).padStart(2, '0')}:00`
    : `${String(gh).padStart(2, '0')}:00 daily`;

  const countdownStr = daysTo > 0
    ? `${daysTo}d ${hrsRem}h ${String(mins).padStart(2, '0')}m`
    : `${hrsRem}h ${String(mins).padStart(2, '0')}m`;
  ...
  return (
    <div className="p-4 border-b border-border bg-card/20">
      ...
      {/* Floor warning */}
      {!aboveFloor && (
        <div className="mb-3 px-3 py-2 rounded-lg bg-yellow-500/10 border border-yellow-500/30 flex items-center gap-2 text-xs text-yellow-400">
          <ShieldCheck className="w-3.5 h-3.5 shrink-0" />
          <span>Win rate <strong>{gwRate}%</strong> is below the 65% floor — bot will not trade this window</span>
        </div>
      )}
        ...
        <div className="flex justify-between items-end">
          <div>
            <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">
              Golden Window {gwWeekday ? "· Day+Hour" : "· Hour"}
            </div>
            <div className="text-2xl font-black font-mono leading-none tracking-tight">
              {slotLabel} <span className="text-xs text-muted-foreground font-sans">UTC</span>
            </div>
            {gwWeekday && (
              <div className="text-[10px] text-yellow-500/80 mt-0.5">Repeats every week</div>
            )}
          </div>
          <div className="text-right">
            <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">Hist. Win Rate</div>
            <div className={`text-lg font-black ${aboveFloor ? "text-yellow-500" : "text-muted-foreground"}`}>{gwRate}%</div>
            {aboveFloor && (
              <div className="text-[9px] text-profit font-semibold">≥65% floor ✓</div>
            )}
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-2">
              {/* Dual-asset confirmation badge */}
              {bothConfirmed ? (
                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-profit/15 border border-profit/30 text-profit flex items-center gap-1">
                  <CheckCircle className="w-2.5 h-2.5" /> BTC+ETH ✓
                </span>
              ) : (
                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-secondary border border-border text-muted-foreground">
                  Need BTC+ETH ≥11/12
                </span>
              )}
                  ...
                  {(session.upScore > 0 || session.downScore > 0) && (
                    <div className="space-y-1">
                      <div className="flex items-center gap-2 text-[10px]">
                        <span className="text-profit w-16">UP  {session.upScore}/12</span>
                        <div className="flex-1 h-1.5 bg-secondary rounded-full overflow-hidden">
                          <div className="h-full bg-profit rounded-full transition-all" style={{ width: `${(session.upScore / 12) * 100}%` }} />
                        </div>
                      </div>
                      <div className="flex items-center gap-2 text-[10px]">
                        <span className="text-loss w-16">DN  {session.downScore}/12</span>
                        <div className="flex-1 h-1.5 bg-secondary rounded-full overflow-hidden">
                          <div className="h-full bg-loss rounded-full transition-all" style={{ width: `${(session.downScore / 12) * 100}%` }} />
                        </div>
                      </div>
                    </div>
                  )}
